...
- [ ] Desktop shortcut **Hair Spa Manager** mở đúng app.
- [ ] Start Menu shortcut **Hair Spa Manager** mở đúng app.
...
## 2) Code signing
- [ ] Máy release có certificate code-signing hợp lệ trong Current User certificate store.
- [ ] `SALON_SIGNING_THUMBPRINT` trỏ đúng certificate; không lưu private key/PFX/password trong repo.
- [ ] `npm run package:update:signed` pass.
- [ ] `npm run verify:installer:signed` xác nhận `salonmanager.exe` và installer đều có Authenticode signature hợp lệ.
...
- [ ] Sau merge, CI trên `main` xanh trên đúng merge SHA.
