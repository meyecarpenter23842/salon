...
Future<void> _openEmployeeEditor(
  ...
  ref.read(employeeSearchQueryProvider.notifier).state = saved['name']?.toString() ?? '';
  ...
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
      content: Text(
        employee == null ? 'Đã thêm nhân sự ${saved['name']}' : 'Đã cập nhật ${saved['name']}',
      ),
    ),
  );
}
...
Future<void> _updateEmployeeStatus(
  ...
  ref.read(employeeSearchQueryProvider.notifier).state = updated['name']?.toString() ?? '';
  ref.read(selectedEmployeeIndexProvider.notifier).state = 0;
  ref.invalidate(filteredEmployeesProvider);
  ref.invalidate(employeesViewProvider);
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(content: Text('Đã chuyển ${updated['name']} sang trạng thái ${updated['status']}')),
  );
}
...
class _EmployeesView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final stylists = items.where((item) => item['role'].toString().contains('Stylist')).length;
    ...
    return LayoutBuilder(
      builder: (context, viewport) {
          ...
          children: [
            PremiumSectionCard(
              ...
              child: PremiumPageHeader(
                ...
                title: 'Nhân viên',
                subtitle: 'Theo dõi vai trò, ca làm, chuyên môn, hiệu suất và trạng thái làm việc của từng thành viên.',
                trailing: [
                  PremiumStatusPill(label: '${items.length} hồ sơ', tone: AppColors.copper),
                  FilledButton.icon(
            ...
            _EmployeeStats(total: items.length, active: active, upcoming: upcoming, stylists: stylists),
            const SizedBox(height: 14),
        ...
        return shortViewport
            ...
            : KeyedSubtree(key: const Key('employees-premium-workspace'), child: page);
      },
    );
  }
}
...
class _EmployeeStats extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final cards = [
      PremiumStatCard(icon: Icons.groups_2_outlined, label: 'Tổng nhân sự', value: '$total'),
      PremiumStatCard(
...
class _EmployeesToolbar extends ConsumerWidget {
  ...
  static const roles = ['Tất cả', 'Stylist chính', 'Barber', 'Chăm sóc tóc', 'Lễ tân'];
        ...
        children: [
          TextFormField(
            ...
            decoration: const InputDecoration(
              ...
              hintText: 'Tìm tên, vai trò, chuyên môn, số điện thoại hoặc trạng thái',
            ),
          ),
      ...
      builder: (context, constraints) {
        if (constraints.maxWidth < 760) {
          return ListView(
            primary: false,
            children: [
              SizedBox(height: 300, child: _EmployeeList(items: items, selectedIndex: selectedIndex)),
              const SizedBox(height: 12),
              SizedBox(height: 390, child: _EmployeeDetail(employee: selected)),
            ],
          );
        }
        return Row(
          ...
          children: [
            Expanded(flex: 11, child: _EmployeeList(items: items, selectedIndex: selectedIndex)),
            const SizedBox(width: 12),
            Expanded(flex: 9, child: _EmployeeDetail(employee: selected)),
          ],
        );
      },
    );
          ...
          : ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                return Material(
                  ...
                  child: InkWell(
                    onTap: () => ref.read(selectedEmployeeIndexProvider.notifier).state = index,
                    child: Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 10),
                      child: Row(
                        children: [
                          PremiumIconBadge(icon: Icons.person_outline_rounded, size: 38, tone: tone),
      ...
      child: Column(
        children: [
          Padding(
            ...
            child: Row(
              children: [
                PremiumIconBadge(icon: Icons.person_rounded, size: 46, tone: tone),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  PremiumInfoRow(icon: Icons.schedule_outlined, label: 'Ca làm', value: current['shift']?.toString() ?? ''),
                  const PremiumDivider(indent: 42),
                  PremiumInfoRow(icon: Icons.auto_awesome_outlined, label: 'Chuyên môn', value: current['specialty']?.toString() ?? ''),
                  const PremiumDivider(indent: 42),
                  PremiumInfoRow(icon: Icons.percent_rounded, label: 'Hoa hồng / KPI', value: current['commission']?.toString() ?? ''),
                  const PremiumDivider(indent: 42),
                  PremiumInfoRow(icon: Icons.trending_up_rounded, label: 'Hiệu suất tháng', value: '${current['monthlyRevenue']} • ${current['todaySchedule']}'),
                  const PremiumDivider(indent: 42),
                  PremiumInfoRow(
                    icon: Icons.sticky_note_2_outlined,
                    label: 'Ghi chú vận hành',
                    value: (current['note']?.toString().isEmpty ?? true) ? 'Chưa có ghi chú' : current['note'].toString(),
                  ),
                ],
              ),
            ),
              ...
              children: [
                Expanded(
                  child: FilledButton.icon(
                    onPressed: () => _openEmployeeEditor(context, ref, employee: current),
                ...
                OutlinedButton.icon(
                  onPressed: () => _updateEmployeeStatus(context, ref, current),
...
class _EmployeeMetricStrip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final metrics = [
      ...
      ('Lịch hôm nay', employee['todaySchedule']?.toString() ?? '-'),
    ];
    return Container(
...
class _EmployeeEditorDialogState extends State<_EmployeeEditorDialog> {
  static const _roleOptions = ['Stylist chính', 'Barber', 'Chăm sóc tóc', 'Lễ tân'];
  ...
  void initState() {
    ...
    _shiftController = TextEditingController(text: employee?['shift']?.toString() ?? '09:00 - 18:00');
    _specialtyController = TextEditingController(text: employee?['specialty']?.toString() ?? '');
    _commissionController = TextEditingController(text: employee?['commission']?.toString() ?? '15%');
    _todayScheduleController = TextEditingController(text: employee?['todaySchedule']?.toString() ?? '0 lịch hôm nay');
    _servicesDoneController = TextEditingController(text: '${employee?['servicesDone'] ?? 0}');
    _monthlyRevenueController = TextEditingController(text: employee?['monthlyRevenue']?.toString() ?? '0đ');
    _ratingController = TextEditingController(text: employee?['rating']?.toString() ?? '4.8');
    _noteController = TextEditingController(text: employee?['note']?.toString() ?? '');
    _role = _roleOptions.contains(employee?['role']) ? employee!['role'].toString() : _roleOptions.first;
    _status = _statusOptions.contains(employee?['status']) ? employee!['status'].toString() : _statusOptions.first;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
              ...
              children: [
                  ...
                  children: [
                    Expanded(
                      child: DropdownButtonFormField<String>(
                        ...
                        items: _roleOptions.map((item) => DropdownMenuItem(value: item, child: Text(item))).toList(),
                        onChanged: (value) {
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 12),
                    Expanded(child: TextFormField(controller: _shiftController, decoration: const InputDecoration(labelText: 'Ca làm'))),
                  ],
                ),
                const SizedBox(height: 12),
                TextFormField(controller: _specialtyController, decoration: const InputDecoration(labelText: 'Chuyên môn')),
                const SizedBox(height: 12),
                Row(
                  children: [
                    Expanded(child: TextFormField(controller: _commissionController, decoration: const InputDecoration(labelText: 'Hoa hồng / KPI'))),
                    const SizedBox(width: 12),
                    Expanded(child: TextFormField(controller: _todayScheduleController, decoration: const InputDecoration(labelText: 'Lịch hôm nay'))),
                  ],
                ),
                const SizedBox(height: 12),
                Row(
                  children: [
                    Expanded(
                      child: TextFormField(
                        ...
                        decoration: const InputDecoration(labelText: 'Dịch vụ tháng'),
                        keyboardType: TextInputType.number,
                      ),
                    ),
                    const SizedBox(width: 12),
                    Expanded(child: TextFormField(controller: _ratingController, decoration: const InputDecoration(labelText: 'Đánh giá'))),
                  ],
                ),
                const SizedBox(height: 12),
                TextFormField(controller: _monthlyRevenueController, decoration: const InputDecoration(labelText: 'Doanh thu tháng')),
                const SizedBox(height: 12),
                TextFormField(controller: _noteController, decoration: const InputDecoration(labelText: 'Ghi chú vận hành'), maxLines: 3),
              ],
            ),
          ),
        ),
      ...
      actions: [
        ...
        FilledButton(onPressed: _submit, child: Text(isEditing ? 'Lưu nhân sự' : 'Tạo nhân sự')),
      ],
    );
  }
...
Color _statusTone(String status) {
  if (status == 'Đang làm việc') return AppColors.success;
  if (status == 'Sắp có lịch') return AppColors.warning;
  if (status == 'Tạm nghỉ') return AppColors.textMuted;
