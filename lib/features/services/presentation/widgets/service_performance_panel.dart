        ...
        data: (detail) {
          if (detail == null) {
            return const _DetailMessage(
              ...
              message: 'Backend demo không có lịch sử giao dịch thật cho dịch vụ.',
            );
          }
...
class _ServicePerformanceBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final history = _mapList(detail['history']).take(5).toList(growable: false);
    final metrics = [
    ...
    return Column(
      ...
      children: [
        ...
        if (topStaff.isEmpty)
          ...
          for (var index = 0; index < topStaff.length && index < 3; index++) ...[
            _StaffRow(index: index + 1, row: topStaff[index]),
            if (index < topStaff.length - 1 && index < 2)
...
class _StaffRow extends StatelessWidget {
  const _StaffRow({required this.index, required this.row});
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        ...
        Expanded(
          child: Text(
            row['employeeName']?.toString() ?? 'Nhân viên',
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
        ...
        Text(
          '${_toInt(row['quantity'])} lượt · ${row['revenue'] ?? '0đ'}',
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${row['employeeName'] ?? 'Chưa gán nhân viên'} · x${_toInt(row['quantity'])} · giá ${row['unitPrice'] ?? '0đ'}',
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
